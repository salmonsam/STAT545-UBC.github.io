...
-summary collapses information in one column (e.g. takes the average of a column)
-mutate will not collapse a column, rather you can do a progression through the data
-so in the example below, if we group_by(country), then we can use mutate to go through and calculate the change in pop from year to year
-also need to use arrange to make sure the data is listed in chronological order
...
-you can also use 'lag' to track the change in pop each year, rather than looking at the change since the first year
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
    geom_point(aes(colour=continent),
               alpha=0.2) +
    scale_colour_discrete("Continents of\n the World") ##the \n creates a new line, so the legend title is over 2 lines
```
...
- The change in population from 1962 to 1972.= aggregate
- The population, in billions. = vectorized
...
gapminder %>%
        ...
        arrange(year) %>%
        group_by(country) %>%
        summarise(popchange = diff(pop))

gapminder %>%
        group_by(country) %>%
        summarise(popchange = pop[year==1972]-pop[year==1962])
```
```{r}
## The population, in billions. = vectorized
gapminder %>%
       mutate(popbil = pop/1000000000)
```
